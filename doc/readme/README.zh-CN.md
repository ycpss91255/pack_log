...
- **传输速度限制**：通过 `--bwlimit <rate>` 限制传输带宽，避免占满共享网络。支持纯 KB/s 数值（`500`）或 `K` / `M` / `G` 后缀（`500K`、`10M`、`1G`，不分大小写，末尾 `B` 可省）。rsync 使用 `--bwlimit`；scp/sftp 使用 `-l`（自动转为 Kbit/s）。
