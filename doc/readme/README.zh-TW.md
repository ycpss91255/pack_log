...
> **TL;DR** — 單檔 Bash 腳本，透過 SSH 連線到遠端主機，依時間範圍尋找 log 檔案，再用 rsync/scp/sftp 傳回本機。Bats + Kcov 測試。
>
> ```bash
...
- **傳輸速度限制**：透過 `--bwlimit <KB/s>` 限制傳輸頻寬，避免佔滿共享網路（rsync 使用 `--bwlimit`；scp/sftp 使用 `-l`，單位為 Kbit/s）。
...
| `--bwlimit <KB/s>` | 限制傳輸速度（KB/s，0 = 不限制） |
