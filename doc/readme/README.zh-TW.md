...
- **傳輸速度限制**：透過 `--bwlimit <rate>` 限制傳輸頻寬，避免佔滿共享網路。支援純 KB/s 數值（`500`）或 `K` / `M` / `G` 後綴（`500K`、`10M`、`1G`，不分大小寫，尾端 `B` 可省）。rsync 使用 `--bwlimit`；scp/sftp 使用 `-l`（自動轉為 Kbit/s）。
