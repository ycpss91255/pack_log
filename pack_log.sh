...
declare SSH_KEY="${HOME}/.ssh/get_log" SSH_TIMEOUT=3
declare TRANSFER_MAX_RETRIES=3 TRANSFER_RETRY_DELAY=5
...
Usage: $(basename "$0") [options]
  Options:
    -n, --number                  Host number (1-${#HOSTS[@]})
    -u, --userhost <user@host>    User and host (e.g. user@host)
    -l, --local                   Use local machine
    -s, --start <YYYYmmdd-HHMMSS>  Start time (e.g. 20260101-000000)
    -e, --end <YYYYmmdd-HHMMSS>    End time (e.g. 20260101-235959)
...
pkg_install_handler() {
  ...
  if command -v "${pkg_name}" >/dev/null 2>&1; then
    log_debug "Package ${pkg_name} already installed."
    return 0
  fi

  log_info "Package ${pkg_name} not found, installing..."

  # Check for sudo access. If missing, return error immediately.
  if ! have_sudo_access; then
    log_error "No sudo access to install ${pkg_name}." # KCOV_EXCL_LINE
  fi
  ...
  if ! { sudo apt-get update && sudo apt-get install -y "${pkg_name}"; }; then
    log_warn "Failed to install package ${pkg_name}."
    return 0
  fi
...
create_folder() {
  ...
  if ! execute_cmd "${mkdir_cmd}"; then
    log_error "Failed to create folder: ${path}" # KCOV_EXCL_LINE
  fi
...
host_handler() {
  ...
  if [[ -z "${NUM}" && -z "${HOST}" ]]; then
    ...
    local input=""
    read -er -p "Enter local, number (1-${#HOSTS[@]}) or user@host: " input
    if [[ "${input,,}" == "local" ]]; then
      log_debug "Using local machine as host"
      HOST="local"
      return 0
      ...
      log_error "Invalid input: ${input}" # KCOV_EXCL_LINE
    fi
  fi
  ...
  if [[ "${NUM}" =~ ^[1-9][0-9]*$ ]]; then
    if (( "${NUM}" < 1 || "${NUM}" > ${#HOSTS[@]} )); then
      log_error "Number must be between 1 and ${#HOSTS[@]}"
    fi
  ...
  if [[ ! "${HOST}" =~ ^[^@[:space:]]+@[^@[:space:]]+$ ]]; then
    log_error "Invalid user@host format: ${HOST}"
  fi
...
time_handler() {
  ...
  for t in START_TIME END_TIME; do
    ...
    if [[ -z "${!t}" ]]; then
      read -er -p "Enter ${t,,} (YYYYmmdd-HHMMSS): " time # KCOV_EXCL_LINE
    else
      time="${!t}"
    fi
...
ssh_handler() {
  ...
  while (( attempt < max_retries )); do
    log_debug "Attempting SSH connection to ${HOST} (attempt $(( attempt + 1 ))/${max_retries})..."
    ...
    if [[ -f "${SSH_KEY}" ]]; then
      log_debug "SSH key ${SSH_KEY} exists"
      case "${err_msg}" in
        *"Permission denied"*)
          log_debug "SSH key permission denied, will attempt to copy key."
          need_copy_key="true"
          ;;
        *"Host key verification failed"*|*"REMOTE HOST IDENTIFICATION HAS CHANGED!"*)
          log_debug "SSH host identification has changed, removing old key."
          need_remove_host="true"
          ;;
        *)
          log_error "SSH connection failed: ${err_msg}"
          ;;
      esac
    else
      log_debug "SSH key ${SSH_KEY} does not exist"
      need_create_key="true"
      need_copy_key="true"
    fi

    if [[ "${need_create_key}" == "true" ]]; then
      log_debug "Creating new SSH key"
      local dsa=""
      for dsa in ed25519 rsa; do
        [[ -f "${SSH_KEY}" ]] && break
        log_info "SSH key ${SSH_KEY} not found, creating with ${dsa}..."
        ssh-keygen -t "${dsa}" -f "${SSH_KEY}" -N "" 2>/dev/null && break
      done
      [[ -f "${SSH_KEY}" ]] || log_error "Failed to create SSH key ${SSH_KEY}"
    fi

    if [[ "${need_remove_host}" == "true" ]]; then
      if ssh-keygen -F "${host_ip}" &>/dev/null; then
        log_info "Removing existing SSH host key for ${host_ip}."
        ssh-keygen -R "${host_ip}" &>/dev/null
      fi
      log_info "Adding ${host_ip} to known hosts."
      ssh-keyscan -H "${host_ip}" >> "${known_hosts}" 2>/dev/null
    fi

    if [[ "${need_copy_key}" == "true" ]]; then
      [[ -f "${SSH_KEY}" ]] || log_error "Private key not found: ${SSH_KEY}"
      [[ -f "${SSH_KEY}.pub" ]] || log_error "Public key not found: ${SSH_KEY}.pub"

      local derived_key
      derived_key=$(ssh-keygen -y -f "${SSH_KEY}" 2>/dev/null | awk '{print $1, $2}') \
        || log_error "Invalid private key: ${SSH_KEY}"

      local file_key
      file_key=$(awk '{print $1, $2}' "${SSH_KEY}.pub")
      [[ "${derived_key}" == "${file_key}" ]] || log_error "Public key does not match private key."

      ssh-copy-id -i "${SSH_KEY}.pub" \
        -o ConnectTimeout="${SSH_TIMEOUT}" -o StrictHostKeyChecking=no \
        "${HOST}" 2>/dev/null || \
      log_debug "Failed to copy SSH key to ${HOST}."
    fi

    log_debug "SSH retry $(( attempt + 1 ))/${max_retries} failed: ${err_msg}"
    (( attempt+=1 ))
  done

  # KCOV_EXCL_START — kcov cannot instrument multi-line command substitution
  log_error "$(
    printf 'SSH connection failed after %s retries:\n' "${max_retries}"
    printf '  %s\n' "${err_msgs[@]}"
  )"
  # KCOV_EXCL_STOP
}
...
get_tools_checker() {
  ...
  for tool in "${tools[@]}"; do
    ...
    if [[ "${tool}" == "rsync" && "${HOST}" != "local" ]]; then
      ...
      if ! execute_cmd "command -v rsync >/dev/null 2>&1"; then
        log_warn "rsync not available on remote host, trying next tool..."
        continue
      fi
      # KCOV_EXCL_STOP
    fi
  ...
  log_error "No file transfer tools (${tools[*]}) available." # KCOV_EXCL_LINE
}
...
folder_creator() {
  ...
  if ! combined=$(execute_cmd "printf '%s_%s' \"\$(hostname)\" \"\$(date +%Y%m%d-%H%M%S)\""); then
    log_error "Failed to get hostname/date from ${HOST}" # KCOV_EXCL_LINE
  fi
...
file_cleaner() {
  ...
  if ! execute_cmd "${rm_cmd}"; then
    ...
    log_debug "Remote folder ${SAVE_FOLDER} removed successfully."
  fi
}
...
file_sender() {
  ...
  log_info "Local destination folder: ${local_save_folder}"

  local remote_esc
  printf -v remote_esc '%q' "${SAVE_FOLDER}"

  if ! execute_cmd "test -d ${remote_esc}"; then
    log_error "Remote folder not found: ${SAVE_FOLDER}"
  fi
  ...
  while (( attempt < TRANSFER_MAX_RETRIES )); do
    ...
    *)
    ...
    if (( attempt < TRANSFER_MAX_RETRIES )); then
      log_warn "${tool} failed (attempt ${attempt}/${TRANSFER_MAX_RETRIES}), retrying in ${TRANSFER_RETRY_DELAY}s..."
...
main() {
  ...
  log_info "=== Step 1/5: Resolving target host ==="
  host_handler

  log_info "=== Step 2/5: Validating time range ==="
  time_handler

  if [[ "${HOST}" != "local" ]]; then
    log_info "=== Step 3/5: Establishing SSH connection ==="
    ssh_handler
    get_tools_checker
  else
  ...
  if [[ "${HOST}" != "local" ]]; then
    log_info "=== Step 5/5: Transferring files to local (${GET_LOG_TOOL}) ===" # KCOV_EXCL_LINE
    file_sender # KCOV_EXCL_LINE
  else
    log_info "=== Step 5/5: Files collected locally ==="
  fi

  log_info "Packaging log completed successfully."
}
